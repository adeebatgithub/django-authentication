...
from compact.utils import get_object_or_redirect
...
class UpdateUser(LoginRequiredMixin, generic.UpdateView):
    ...
    def get_success_url(self):
        return reverse_lazy("compact:profile", kwargs={"username": self.object.username})
    ...
    def get(self, request, *args, **kwargs):
        if request.user.username != self.kwargs.get("username"):
            pattern = f"compact:{resolve(request.path).url_name}"
