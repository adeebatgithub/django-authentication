...
from users.utilities.token.user_token import PathTokenValidationMixin, token_generator, TokenValidationMixin
...
class RedirectUser(LoginRequiredMixin, generic.RedirectView):
    ...
    """
    otp_pattern_name = "users:verification-send-mail-otp"
...
class VerificationSendLinkMail(LoginRequiredMixin, PathTokenValidationMixin, mail_views.SendEmailView):
    ...
    def get_email_context_data(self):
        url = generate_uidb64_url(
            pattern_name="users:verification-update-status", user=self.request.user,
